import torch, GPUtil as gpu, math, torchcrepe, torchaudio, os, re, tqdm
...
def get_predictions(x, sr, hop_length, fmin, fmax, batch_size, device, avg_factor = 3, remove_quant=True, thresh=1.e-2):
    if x.ndim == 2:
        frequency = []
        periodicity = []
        for x_tmp in x:
...
def parse_predictions(frequency, periodicity, hop_length, sr, batch_size, threshold):
    count = 0
    pitch_str = ""
    octave_str = ""
...
def extract_pitch(input, hop_length, fmin, fmax, threshold, batch_size): 
    ...
    assert os.path.isdir(input), "%s does not seem to be a file"%input
    input+="/"

    input_dir = join(input, "data")
    pitch_metadata_dir = join(input, "metadata", "pitch")
    octave_metadata_dir = join(input, "metadata", "octave")
    pitch_metadata_tracks_dir = join(input, "metadata", "pitch", "tracks")
    octave_metadata_tracks_dir = join(input, "metadata", "octave", "tracks")
    ...
    for current_file in tqdm.tqdm(valid_files, total=len(valid_files), desc="Extracting pitch from files..."):
        ...
        frequency, periodicity = get_predictions(x, sr, hop_length, fmin, fmax, batch_size=batch_size, device=device, thresh=threshold)
        pitch_string, octave_string = parse_predictions(frequency, periodicity, hop_length, sr, batch_size, threshold)
        current_file_path = re.sub(os.path.abspath(input)+"/", '', current_file)
